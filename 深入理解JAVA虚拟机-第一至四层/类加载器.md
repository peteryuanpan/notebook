...
以JDBC来举例，java.sql.Driver是mysql的接口，

#### 线程上下文类加载器的定义

SPI 的接口是Java核心库的一部分，是由启动类加载器（Bootstrap Classloader）来加载的，但是 SPI 的实现类是由系统类加载器（System ClassLoader）。前文说了，系统类加载器默认是应用程序类加载器
