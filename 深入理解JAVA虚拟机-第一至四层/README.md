...
经过总结，统一解法满足以下节点
- 规则一：main函数所在类先进行加载，结束后调用main方法
- 规则二：加载一个类之前，会先加载该类的父类
- 规则三：默认情况下，已经加载过的类不会加载（非默认情况是指自定义类加载器）
- 规则四：类加载会在“准备”阶段，执行static修饰的代码块（static{}），对static修饰的域（field，即成员变量）进行赋值
- 规则五：类加载执行顺序与代码书写顺序一致
- 规则六：当遇到new、getstatic、putstatic 或 invokestatic 这4条字节码指令时，会插入结算新的类加载
