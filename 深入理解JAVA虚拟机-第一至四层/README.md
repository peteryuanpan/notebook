...
准备阶段正式为静态变量（static修饰的）分配内存于堆区，并设置静态变量的初始值。这里不包括普通实例变量

这里的初始值一般情况下是指零值，除了被final修饰的以外
...
static、final同时修饰的变量，不是赋值零值，而是赋初值
...
- 规则三：默认情况下，已经加载过的类不会加载（非默认情况是指自定义类加载器）
- 规则四：类加载会在“准备”阶段，执行静态代码块，对静态变量赋零值，但若又有final修饰会赋初值
- 规则五：类加载执行顺序与代码书写顺序保持一致
- 规则六：当遇到new、getstatic、putstatic或invokestatic这4条字节码指令时，会插入结算新的类加载，再执行指令，再继续旧的类加载
...
上面的例子证明了统一解法的合理性。你能举出反例吗？若举得出来，欢迎告诉我，有赏哦
