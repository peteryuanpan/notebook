...
- 规则二：默认情况下，已经加载过的类不会加载（非默认情况是指自定义类加载器）
- 规则三：类加载会在“准备”阶段，执行static修饰的代码块（static{}），对static修饰的域（field，即成员变量）进行赋值
- 规则四：类加载执行顺序与代码书写顺序一致
- 规则五：当遇到new、getstatic、putstatic 或 invokestatic 这4条字节码指令时，会进行类加载且插入结算
