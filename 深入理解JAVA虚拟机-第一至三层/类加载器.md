...
因此，调用了myLoader的loadClass方法，执行了System.out.println("JDBCMysqlTest1 " + name); 输出了JDBCMysqlTest1 com.mysql.cj.jdbc.Driver

相信到了这里，讲明白了线程上下文加载器的含义 及 一个用法例子

### 类加载与Tomcat

Tomcat是一套Java Web服务器架构，主流的Java Web服务器，如Tomcat、Jetty、WebLogic、WebSphere等，都实现了自己定义的类加载器（一般都不止一个）

一个功能健全的Web服务器，要解决如下几个问题
- 部署在同一个服务器上的两个Web应用程序所使用的Java类库可以实现相互隔离
- 部署在同一个服务器上的两个Web应用程序锁使用的Java类库可以相互共享
- 服务器需要尽可能的保证自身安全不受部署的Web应用程序影响，服务器所使用的类库应该与应用程序的类库相互独立
- 主流的Web服务器都会支持JSP生成类的热加载

第一点，是最基本的需求，两个不同的应用程序可能会依赖同一个第三方类库的不同版本，因此需要独立使用，相互隔离；第二点，比如用户可能有10个Spring组织的应用程序部署在同一台服务器上，如果都放在各个应用程序的隔离目录中，将会是很大的资源浪费，不仅仅是磁盘空间的问题，类加载过多容易导致方法区溢出；第三点，是处于服务器本身安全考虑的；第四点，JSP文件最终要编译成Java Class才能由虚拟机执行，但JSP文件由于其纯文本存储的特性，运行时修改的概率很大，因此需要热加载，网页应用无需重启也能修改

在Tomcat5.x版本中，有4种目录可以存放Java类库，分别是 /common/、/server/、/shared/ ，还加上Web应用程序自身的目录 /WEB-APP/WEB-INF/
- 放置在 /common/ 目录中，类库可被Tomcat和所有Web应用程序共同使用
- 放置在 /server/ 目录中，类库可被Tomcat使用，对所用的Web应用程序都不可见
- 放置在 /shared/ 目录中，类库可被所有Web应用程序共同使用，但对Tomcat自己不可见
- 放置在 /WebApp/WEB-INF/ 目录中，类库仅可以被此WebApp使用，对Tomcat和其他Web应用程序都不可见

根据上面几种情况，可以画出Tomcat服务器的类加载架构（正统的类加载器架构）

![image](https://user-images.githubusercontent.com/10209135/96210583-cc34d600-0fa4-11eb-818b-47c7af4a3438.png)

类加载器逻辑如下
- CommonClassLoader加载/common/中的类库，且加载的类可以被CatalinaClassLoader和SharedClassLoader使用
- CatalinaClassLoader加载/server/中的类库，且与SharedClassLoader加载的类相互隔离，不可相互使用
- SharedClassLoader加载/shared/中的类库，且加载的类可以被WebpAppClassLoader使用，但与CatalinaClassLoader加载的类相互隔离，不可相互使用
- WebAppClassLoader加载/WebApp/WEB-INF/中的类库，且各个WebAppClassLoader加载的类相互隔离，不可相互使用
- 每一个JSP文件对应一个JSP类加载器，且JasperLoader加载的范围仅仅是这个JSP文件所编译出来的那一个Class，它出现的目的就是为了被丢弃：当服务器检测到JSP文件被修改时，会替换掉目前的JasperLoader的实例，并通过再建立一个新的JSP类加载器来实现JSP文件的HotSwap功能
- 类加载器逻辑符合双亲委派模型

在Tomcat6.x版本中，情况有所变化，只有指定了tomcat/conf/catalina.properties配置文件的server.loader和share.loader项后才会真正建立CatalinaClassLoader和SharedClassLoader的实例，否则会用到这两个类加载的地方都会用CommonClassLoader的实例代替，而默认的配置文件中没有设置这两个loader项，所以Tomcat6.x顺利成章地把/common/、/server、/shared/三个目录默认合并到一起变成一个/lib/目录，这个目录的类库相当于以前/common/目录中类库的作用。这是Tomcat设计团队为了简化大多数的部署场景所做的一项改进，如果默认设置不能满足需要，用户可以通过修改配置文件制定server.loader和share.loader的方式重新启用Tomcat5.x的类加载器架构

提一个问题，如果有10个Web应用程序都是用Spring类进行组织和管理的话，可以把Spring放到Common或Shared目录下让这些程序共享。Spring要对应用程序的类进行管理，自然要能访问到用户程序的类，而用户的程序显然是放在/WebApp/WEB-INF/目录中的，那么被CommonClassLoader或SharedClassLoader所加载的Spring如何访问并不在其加载范围内的用户程序呢。答案当然是破坏双亲委派模型，使用自定义类加载的方式或者SPI机制的方式来实现
