- [前言](#前言)
- [JAVA虚拟机的定义](#java虚拟机的定义)
- [JAVA语言的水深](#java语言的水深)
- [如何学习JVM](#如何学习jvm)
- [项目成果](#项目成果)
- [文章目录](#文章目录)
  - [第0章：jvm基础概念](#第0章jvm基础概念)
  - [第1章：类加载机制与类加载器](#第1章类加载机制与类加载器)
  - [第2章：类文件结构与字节码指令](#第2章类文件结构与字节码指令)
  - [第3章：运行时数据区域](#第3章运行时数据区域)
  - [第4章：字节码执行引擎](#第4章字节码执行引擎)
  - [第5章：对象的生命周期](#第5章对象的生命周期)
  - [第6章：垃圾收集机制](#第6章垃圾收集机制)
  - [第7章：JVM性能调优](#第7章jvm性能调优)
  - [第8章：JVM面试题](#第8章jvm面试题)
- [思维导图](#思维导图)
  - [类加载机制](#类加载机制)
  - [运行时数据区域](#运行时数据区域)
  - [垃圾收集机制](#垃圾收集机制)
...
并发的概念需要涉及到操作系统原理、CPU原理，将在[深入理解JAVA并发与集合](../深入理解JAVA并发与集合)文章集中总结
...
学习方法
- 学习了一块知识点后，自己梳理成文章总结（这是第一步，更优先）
- 然后画思维导图，将内容讲出来，一遍一遍地过，直到能让人听明白为止（这是第二步，十分重要）
- 注意，写出来和说出来对于理解的提高是非常不一样的，写完后必须说出来，讲明白了，就通透了
...
- [JVM基础概念](JVM基础概念.md)
  ...
  - [InstanceKlass](JVM基础概念.md#InstanceKlass)
    - [InstanceMirrorKlass](JVM基础概念.md#InstanceMirrorKlass)
    - [InstanceRefKlass](JVM基础概念.md#InstanceRefKlass)
  - [ArrayKlass](JVM基础概念.md#ArrayKlass)
    - [TypeArrayKlass](JVM基础概念.md#TypeArrayKlass)
    - [ObjArrayKlass](JVM基础概念.md#ObjArrayKlass)
  - [oopDesc](JVM基础概念.md#oopDesc)
    - [instanceOopDesc](JVM基础概念.md#instanceOopDesc)
    - [arrayOopDesc](JVM基础概念.md#arrayOopDesc)
      - [typeArrayOopDesc](JVM基础概念.md#typeArrayOopDesc)
      - [objArrayOopDesc](JVM基础概念.md#objArrayOopDesc)
...
#### 第4章：字节码执行引擎
- [字节码执行引擎](字节码执行引擎.md)
  - [字节码执行引擎的定义](字节码执行引擎.md#字节码执行引擎的定义)
  - [虚拟机栈与本地方法栈](字节码执行引擎.md#虚拟机栈与本地方法栈)
  - [类加载与方法运行](字节码执行引擎.md#类加载与方法运行)
  - [运行时栈帧结构](字节码执行引擎.md#运行时栈帧结构)
    - [局部变量表](字节码执行引擎.md#局部变量表)
    - [操作数栈](字节码执行引擎.md#操作数栈)
    - [动态链接](字节码执行引擎.md#动态链接)
    - [返回地址](字节码执行引擎.md#返回地址)
  - [方法调用](字节码执行引擎.md#方法调用)
    - [解析调用](字节码执行引擎.md#解析调用)
    - [分派调用](字节码执行引擎.md#分派调用)
      - [静态分派与方法重载](字节码执行引擎.md#静态分派与方法重载)
      - [动态分派与方法重写](字节码执行引擎.md#动态分派与方法重写)
      - [虚方法表与接口方法表](字节码执行引擎.md#虚方法表与接口方法表)
    - [invokedynamic指令](字节码执行引擎.md#invokedynamic指令)
  - [方法执行](字节码执行引擎.md#方法执行)
    - [基于字节码解释器的解释执行](字节码执行引擎.md#基于字节码解释器的解释执行)
    - [基于模板解释器及即时编译器的混合模式](字节码执行引擎.md#基于模板解释器及即时编译器的混合模式)
    - [即时编译技术](字节码执行引擎.md#即时编译技术)
  - [JAVA是半编译半解释型语言](字节码执行引擎.md#JAVA是半编译半解释型语言)

#### 第5章：对象的生命周期
- [对象的生命周期](对象的生命周期.md)
  - [对象的创建过程](对象的生命周期.md#对象的创建过程)
    - [字节码指令new](对象的生命周期.md#字节码指令new)
    - [字节码解释器与new](对象的生命周期.md#字节码解释器与new)
    - [类加载检查](对象的生命周期.md#类加载检查)
    - [内存分配](对象的生命周期.md#内存分配)
      - [指针碰撞](对象的生命周期.md#指针碰撞)
      - [空闲列表](对象的生命周期.md#空闲列表)
      - [TLAB技术](对象的生命周期.md#TLAB技术)
    - [初始化零值](对象的生命周期.md#初始化零值)
    - [设置对象头](对象的生命周期.md#设置对象头)
    - [执行init方法](对象的生命周期.md#执行init方法)
  - [对象的内存布局](对象的生命周期.md#对象的内存布局)
    - [对象头](对象的生命周期.md#对象头)
      - [MarkWord](对象的生命周期.md#MarkWord)
      - [类型指针](对象的生命周期.md#类型指针)
      - [数组长度](对象的生命周期.md#数组长度)
    - [实例数据](对象的生命周期.md#实例数据)
    - [对齐填充](对象的生命周期.md#对齐填充)
    - [计算对象大小](对象的生命周期.md#计算对象大小)
    - [指针压缩](对象的生命周期.md#指针压缩)
  - [对象的访问定位](对象的生命周期.md#对象的访问定位)
    - [句柄访问](对象的生命周期.md#句柄访问)
    - [直接指针](对象的生命周期.md#直接指针)

#### 第6章：垃圾收集机制
- [垃圾收集机制](垃圾收集机制.md)
  - [垃圾收集算法](垃圾收集机制.md#垃圾收集算法)
    - [分代收集理论](垃圾收集机制.md#分代收集理论)
    - [标记清除算法](垃圾收集机制.md#标记清除算法)
    - [标记整理算法](垃圾收集机制.md#标记整理算法)
    - [标记复制算法](垃圾收集机制.md#标记复制算法)
    - [引用计数算法](垃圾收集机制.md#引用计数算法)
    - [可达性分析算法](垃圾收集机制.md#可达性分析算法)
    - [GCRoots与安全点](垃圾收集机制.md#GCRoots与安全点)
    - [StopTheWorld](垃圾收集机制.md#StopTheWorld)
  - [垃圾收集器](垃圾收集机制.md#垃圾收集器)
    - [Serial收集器](垃圾收集机制.md#Serial收集器)
    - [ParNew收集器](垃圾收集机制.md#ParNew收集器)
    - [ParallelScavenge收集器](垃圾收集机制.md#ParallelScavenge收集器)
    - [CMS收集器](垃圾收集机制.md#CMS收集器)
    - [G1收集器](垃圾收集机制.md#G1收集器)
  - [理解GC日志](垃圾收集机制.md#理解GC日志)
  - [内存分配与回收策略](垃圾收集机制.md#内存分配与回收策略)
    - [对象优先在Eden区分配](垃圾收集机制.md#对象优先在Eden区分配)
    - [长期存活的对象进入老年代](垃圾收集机制.md#长期存活的对象进入老年代)
    - [大对象直接进入老年代](垃圾收集机制.md#大对象直接进入老年代)
    - [动态年龄判断](垃圾收集机制.md#动态年龄判断)
    - [空间分配担保](垃圾收集机制.md#空间分配担保)
    - [栈上分配策略](垃圾收集机制.md#栈上分配策略)

#### 第7章：JVM性能调优
- [JVM性能调优](JVM性能调优.md)
  - [基础故障处理工具](JVM性能调优.md#基础故障处理工具)
    - [jps-虚拟机进程状况工具](JVM性能调优.md#jps-虚拟机进程状况工具)
    - [jstat-虚拟机统计信息监视工具](JVM性能调优.md#jstat-虚拟机统计信息监视工具)
    - [jinfo-Java配置信息工具](JVM性能调优.md#jinfo-Java配置信息工具)
    - [jmap-Java内存映像工具](JVM性能调优.md#jmap-Java内存映像工具)
    - [jhat-虚拟机堆转储快照分析工具](JVM性能调优.md#jhat-虚拟机堆转储快照分析工具)
    - [jstack-Java堆栈跟踪工具](JVM性能调优.md#jstack-Java堆栈跟踪工具)
  - [进阶故障处理工具](JVM性能调优.md#进阶故障处理工具)
    - [HSDB-基于服务性代理的调试工具](JVM性能调优.md#HSDB-基于服务性代理的调试工具)
    - [JConsole-Java监视与管理控制台](JVM性能调优.md#JConsole-Java监视与管理控制台)
    - [VisualVM-多合一故障处理工具](JVM性能调优.md#VisualVM-多合一故障处理工具)
    - [Arthas-Alibaba开源的Java诊断工具](JVM性能调优.md#Arthas-Alibaba开源的Java诊断工具)
  - [性能调优记录](JVM性能调优.md#性能调优记录)
    - [程序死循环问题排查](JVM性能调优.md#程序死循环问题排查)
    - [程序死锁问题排查](JVM性能调优.md#程序死锁问题排查)
    - [生成与分析Dump文件](JVM性能调优.md#生成与分析Dump文件)
    - [OOM异常问题排查](JVM性能调优.md#OOM异常问题排查)
    - [线上业务日志切面](JVM性能调优.md#线上业务日志切面)
    - [CPU占用过高问题排查](JVM性能调优.md#CPU占用过高问题排查)
    - [亿级流量系统调优案例](JVM性能调优.md#亿级流量系统调优案例)

#### 第8章：JVM总结
- [JVM总结](JVM总结.md)
  - [为什么说JAVA是一种半编译半解释型语言](JVM总结.md#为什么说JAVA是一种半编译半解释型语言)
  - [什么情况下需要破坏双亲委派模型](JVM总结.md#什么情况下需要破坏双亲委派模型)
  - [GCRoots的作用以及哪些引用可以作为GCRoots](JVM总结.md#GCRoots的作用以及哪些引用可以作为GCRoots)
  - [说一下对象内存分配及进入老年代的策略](JVM总结.md#说一下对象内存分配及进入老年代的策略)
  - [说下常用垃圾收集器的工作原理](JVM总结.md#说下常用垃圾收集器的工作原理)
  - [如何选择一款垃圾收集器](JVM总结.md#如何选择一款垃圾收集器)
  - [GC调优策略经验总结](JVM总结.md#GC调优策略经验总结)
