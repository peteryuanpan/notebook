...
输出是从类文件转化的类的元信息数据，写入内存区域，比如InstanceKlass到方法区，InstanceMirrorKlass到堆区

> 在《深入理解JAVA虚拟机》第二版（7.3.1节）中，InstanceMirrorKlass（即java.lang.Class对象）是写在方法区中的，这是JDK7的逻辑；而在《深入理解JAVA虚拟机》第三版（7.3.1节）中，InstanceMirrorKlass是写在堆内存中的，这是JDK8的逻辑
