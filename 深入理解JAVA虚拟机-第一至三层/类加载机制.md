...
public class ArraysTest2 {
    ...
    public static void main(String[] args) {
        A[] a = new A[5];
        for (int i = 0; i < 5; i ++) {
            System.out.println(System.identityHashCode(a[i]));
        }
        for (int i = 0; i < 5; i ++) {
            a[i] = new A();
            System.out.println(System.identityHashCode(a[i]));
...
数组对象内部具体是怎么样子的结构呢。我目前没有深入了解，不过可以根据经验，进行下估计猜想。数组类内部都有一个length属性，这是已知的，用于表示数组的长度，那么可以将数组理解为一个列表，它每个元素保存的是指向元素类对象的引用（也就是内存地址），起初new一个数组时，每个元素引用都为null，需要赋值。因此new一个数组类时，并不会对元素类进行类加载
