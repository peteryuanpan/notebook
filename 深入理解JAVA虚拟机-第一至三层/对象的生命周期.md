...
未开启指针压缩，对象大小（48B）= MarkWord（8B）+ 类型指针（4B）+ int类型（4B）+ 普通类引用（4B）+ 基本类型数组引用（4B） + short类型（2B）+ 对齐填充（6字节）

通过这个例子可以看出，父类对象的字段（包括private修饰的），也算在子类对象的实例数据中

但static修饰的类数据（无论在子类还是父类）都不属于实例数据，类数据是存储在class对象中的，因此如果一个变量可以声明为static，则声明它，在大量对象的场景下，能节省内存空间
