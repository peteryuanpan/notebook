...
32位机子能存储多大数据，是4GB（4 * 1024 * 1024 * 1024 = 2^2 * 2^10 * 2^10 * 2^10 = 2^32），这种情况下一个oop占32位，4个字节，寻址能力是4GB（这里oop含义是Java对象引用，强调的是指向堆中对象的指针）

64位机子，寻址能力是非常非常大，同样的，一个oop占64位，8个字节，寻址能力也非常非常大

但这样会有多余问题
- 64位增加了GC开销，64位对象引用需要占用更多的堆空间，留给其他数据的空间将会减少，从而加快了GC的发生，更频繁的进行GC
- 降低了CPU缓存命中率，64位对象引用增大了，CPU能缓存的oop将会更少，从而降低了CPU缓存的效率
...
因此，在64位机子下，一个oop统一使用8字节来存储，是不合适的，考虑效率因素，一般依然采用32位来存储，这就是指针压缩了！

指针压缩下，将原本64位的Java对象指针压缩成32位的，因此类型指针、引用类型（包括普通类引用、基本数组类引用、引用数组类引用）都从8个字节压缩成了4个字节，节省了内存，加快了效率

那么问题又来了，使用32位存储一个oop，最大寻址能力是4GB，这个有点小了，怎么扩大呢
