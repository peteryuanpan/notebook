...
TLAB是线程的一块私有内存，即这块buffer只给当前线程使用。意思是，只有当前线程可以向这块内存写入数据，但是写入的数据依然是线程共享的，即TLAB只有当前线程可写，所有线程可读

JDK6以后TLAB默认是开启的，可以通过参数-XX:+/-UseTLAB设置开启或关闭，建议开启。TLAB是从Eden区分出来的，属于堆区内存
