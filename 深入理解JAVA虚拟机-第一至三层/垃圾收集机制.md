...
CMS收集器，全名 Concurrent Mark Sweep，直译是并发标记清除，从名字中就可以看出来，CMS收集器是用的标记-清除算法的，它是老年代的一款垃圾收集器，且是真正意义上第一款并发垃圾收集器，能通过三色标记算法 + 增量更新，做到并发标记的同时不停止用户线程（STW），但同时这也带来了一定的问题。总的来说，CMS是存在一定的缺点，但它的优点仍然比前面几款垃圾收集器要突出，它是许多真实项目（尤其是内存要求较低一些）的非常合适的一款垃圾收集器。可以通过加VM参数 -XX:+UseConcMarkSweepGC 来使用 CMS 收集器
