...
> - 第一部分，垃圾收集算法。我将先从分代收集理论讲起，这是目前几乎所有垃圾收集器的共同思想，它包含了三个假说。再谈到3个最基础的垃圾收集算法：标记清除算法、标记整理算法、标记复制算法。这3个算法是所有后续算法与实现的基础。随之介绍两个基础的标记算法：引用计数算法、可达性分析算法。最后是Hotspot虚拟机在实现垃圾收集过程中考虑到的技术点：GCRoots与安全点、StopTheWorld、并发三色标记、记忆集与卡表等
...
垃圾收集有术语：PartialGC、MniorGC / YoungGC、MajorGC / OldGC、MixedGC、FullGC。解释：PartialGC 指目标不是完整收集整个Java堆的垃圾收集，又分为 MniorGC 和 MajorGC，分别是指新生代的垃圾收集和老年代的垃圾收集。MixedGC 指目标是收集整个新生代以及部分老年代的垃圾收集，目前只有G1收集器会有这种行为。FullGC 指目标是整个Java堆（甚至包括方法区）的垃圾收集

垃圾收集的时机，一般 MinorGC 是当 Eden 区空间内存不够分配时触发，MajorGC 或 FullGC 是当老年代空间内存不够分配时触发。而垃圾收集时机必须还要讨论垃圾收集器，比如CMS收集器在JDK5时会在老年代使用了68%时触发垃圾收集，在JDK6时会在老年代使用了92%时触发垃圾收集。除此外，代码中还可以主动使用 System.gc()，来触发一次 FullGC
...
可以通过加VM参数 -XX:+UseSerialGC 来使用 Serial 收集器

#### ParNew收集器

可以通过加VM参数 -XX:+UseParNewGC 来使用 ParNew 收集器

#### ParallelScavenge收集器

可以通过加VM参数 -XX:+UseParallelGC 来使用 ParallelScavenge 收集器

#### CMS收集器

可以通过加VM参数 -XX:+UseConcMarkSweepGC 来使用 CMS 收集器
...
可以通过加VM参数 -XX:+UseG1GC 来使用 G1 收集器
