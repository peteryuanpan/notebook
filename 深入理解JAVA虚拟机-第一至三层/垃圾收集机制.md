...
> - 第二部分，垃圾收集器。这一部分介绍多个垃圾收集器，可能有Serial、ParNew、Parallel、CMS、G1、ZGC。介绍多个垃圾收集器的优缺点，如何选取垃圾收集器策略等
...
垃圾收集的时机，一般 MinorGC 是当 Eden 区空间内存不够分配时触发，MajorGC 或 FullGC 是当老年代空间内存不够分配时触发。而垃圾收集时机必须要讨论垃圾收集器，比如CMS收集器在JDK5时会在老年代使用了68%时触发垃圾收集，在JDK6时会在老年代使用了92%时触发垃圾收集
