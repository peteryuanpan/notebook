...
其中volatile和transient比较陌生，volatile将在第8章JAVA多线程高并发章节中讲解

而transient的一个例子是HashMap，HashMap中定义几个字段时，使用了transient来修饰，这一块将在容器集合章节集中讲解
...
可以看出来，方法表结构与字段表结构是一致的，都包含访问标志、方法的简单名称、方法的描述符、方法表的属性表个数、方法表的属性表
