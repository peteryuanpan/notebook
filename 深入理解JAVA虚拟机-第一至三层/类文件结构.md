...
|ACC_SYNCHRONIZED|0x0020|方法是否声明为synchronized|
...
其中synchronized和strictfp比较陌生，synchronized将在并发专题中展开，strictfp与前面所讲的IEEE754有关系
...
> 后面我对synchronized及strictfp更熟悉后，可能会在这里增加一个HEADER来专门讲解
...
exception_table表示异常处理表，它主要描述代码中try-catch-finally语句的逻辑，具体解释在下文会提及

exception_table的结构如下
|类型|名称|数量|
|--|--|--|
|U2|start_pc|1|
|U2|end_pc|1|
...
通过test1方法再解释一下exception_table的逻辑
- startpc表示开始行，endpc表示结束行，但不包括endpc
- 0行到10行及21行是try语句
```java
try {
    t = new T1(1);
    return t;
```
- 11行到20行是try语句正常执行之后的finally语句
```java
} finally {
    t = new T1(3);
}
```
- 0到10行若有java.lang.Exception异常，会跳到22行开始执行
- 0到10行若有其他异常，会跳到45行开始执行
- 22行到33行及44行是catch语句
```
} catch (Exception e) {
    t = new T1(2);
    return t;
```
- 34行到43行是catch语句正常执行后的finally语句
- 22行到33行若有任何异常，会跳到45行开始执行
- 45到58行是try语句有其他异常及catch语句有任何异常后执行的finally语句
- 58行会抛出对应异常

有一个比较容易“不清楚”的地方是，test1方法返回的是new T1(1)的对象，而不是new T1(3)的对象，而test2方法返回的是new T1(3)的对象

原因是try语句中若有return语句，执行return前，会先推送return的本地变量到栈顶（称为returnValue），再执行finally方法，若finally方法中有return，则直接返回，若没有，会将returnValue从栈顶取回，再return之

从test3方法来看，推送的returnValue和取回的returnValue都是List x，因此输出[1, 3]

最终，对try-catch-finally语句逻辑，做一个最终总结

try语句
- try语句中无异常
  - 若try语句中无return及throw语句，会执行finally语句
  - 若try语句中有throw语句，会先执行finally语句，若finally语句中没有结束，会回来执行throw语句并结束
  - 若try语句中有return语句，会先保存returnValue到栈顶，执行finally语句，若finally语句中没有结束，会从栈顶取回returnValue，返回之并结束
- try语句中有异常且异常被catch语句捕获，会执行catch语句
- try语句中有异常且异常未被catch语句捕获，会执行finally语句

catch语句
- 若catch语句中无异常
  - 若catch语句中无return及throw语句，会执行finally语句
  - 若catch语句中有throw语句，会先执行finally语句，若finally语句中没有结束，会回来执行throw语句并结束
  - 若catch语句中有return语句，会先保存returnValue到栈顶，执行finally语句，若finally语句中没有结束，会从栈顶取回returnValue，返回之并结束
- 若catch语句中有异常，会执行finally语句

finally语句
- 若finally语句中有return或throw语句，会执行
  - 若是从try或catch的return或throw语句而来，执行完，会回去执行return或throw语句
  - 否则，直接结束
- 若finally语句中无return及throw语句，会执行完并结束

#### ConstantValue

> 在《深入理解JAVA虚拟机》第二版6.3.7节 6.ConstantValue属性中，描述是，ConstantValue属性的作用是通知虚拟机自动为静态变量赋值，只有被static关键字修饰的变量（类变量）才可以使用这项属性，我特意查了第三版，描述没有变化。还提及，目前SunJavac编译器选择是，如果同时使用final和static来修饰一个变量（按照习惯，称为“常量”），并且这个变量的数据类型是基本类型或者java.lang.String的话，就生成ConstantValue属性来进行初始化，如果这个变量被static修饰了，但没有被final修饰，或者并非基本类型及字符串，则将会在clinit方法中进行初始化

我测试出来，只有final修饰的变量，才会带上ConstantValue属性
...
用jclasslib查看，可以很明显看出来，只有a和c有ConstantValue属性，b没有
![image](https://user-images.githubusercontent.com/10209135/95937635-cefbc380-0e0a-11eb-9e9c-a952b77e2e62.png)
...
constantvalue_index是一个指向常量池的索引，指向的常量池项可以是下面5中类型中的一种
- CONSTANT_Long_info
- CONSTANT_Float_info
...
使用ParseClassFile项目解析final int int5 = 987;，得到如下结果
```
[20] Field_Info(0x00100092008e0001009300000002009403000003db)
access_flag: [Field_Access_Flag(0x0010), value: [16(0x0010)], FLAGs: [[ACC_FINAL]]]
name_index: [146(0x0092)]
...
InnterClasses属性用于记录内部类与宿主类之间的关联，如果一个类中定义了内部类，那么编译器将会为它以及它所包含的内部类生成InnerClasses属性
...
数据项number_of_classes代表需要记录多少个内部类信息，每个内部类信息都由一个inner_classes_info表进行描述
...
inner_class_access_flags是内部类的访问标志，这是最后一个访问标志了，先前介绍了类的访问标志、字段表的访问标志、属性表的访问标志

#### 内部类访问标志

|标志名称|标志值|含义|
|--|--|--|
|ACC_PUBLIC|0x0001|内部类是否声明为public|
|ACC_PRIVATE|0x0002|内部类是否声明为private|
|ACC_PROTECTED|0x0004|内部类是否声明为protected|
|ACC_STATIC|0x0008|内部类是否声明为static|
|ACC_FINAL|0x0010|内部类是否声明为final|
|ACC_INTERFACE|0x0020|内部类是否为接口|
|ACC_ABSTRACT|0x0400|内部类是否声明为abstract|
