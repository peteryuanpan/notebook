...
除了 CONSTANT_Dynamic_info、CONSTANT_Module_info、CONSTANT_Package_info 这三个常量池项外

其余14个常量池项，我都把它在一个Java类当中体现出来了，它的源码如下，这份文件会有一些长度，但非常有用
...
> 接下来的皆是过程是非常繁琐的，就如文章最开头所说一样，解析类文件结构，实际上是对JAVA语法更深入的理解

> 为了不过分浪费篇幅，我决定用“点到为止”的办法来解释，比如对于Constant_Utf8_info、Constant_Class_Info这些常见的常量池项，就用几个简单例子来解释即可，而对于CONSTANT_InvokeDynamic_info这样罕见的常量池项，说明它与泛型有关联即可了，实际上理解它需要很长的篇幅（《深入理解JAVA虚拟机》中完整解释用了4-6页），我不想弄那么麻烦，毕竟对于理解JVM原理来说，它不算重点，而是属于细枝末节
