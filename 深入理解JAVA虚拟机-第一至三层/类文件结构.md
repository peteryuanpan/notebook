...
在常量池结束后，紧接着两个字符代表方法标志（access_flags），它表示类或接口的许多状态集合（二进制或运算）
...
接下来有三个数据，分别是类索引（this_class）、父类索引（super_class）、索引集合（interfaces），用于描述类的继承关系（注意，这里所说的“类”也包括接口），这三者的索引都是指向常量池中的某一个类型为Constant_Utf8_Info的项
...
接下来是接口计数器，用于描述实现的接口的个数，若没有实现接口，则计数器值为0，后面的索引集合也不占任何字节

接口索引集合是一组U2类型数据的集合，用来描述类实现了哪些接口，这些接口将按implements语句顺序排列，而如果这个类本身就是一个接口，按照extends语句的顺序排列
