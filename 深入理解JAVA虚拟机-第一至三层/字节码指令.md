...
> 本章与[类文件结构](类文件结构.md)是息息相关的，在类文件结构中，我花了大量的篇幅去解释一个类文件中，每个模块的组成部分、二进制数据等。字节码指令属于方法表中的一个属性（Code），它主要描述了方法中的语句

> 解释下操作码。举个例子，字节码 new x，操作码就是new；字节码 getstatic x，操作码就是 getstatic。不过，实际上类文件中，操作码并不是new、getstatic这样的字符串，而是这个字节码的索引号，或者说字节码指令表中的指令码，new的索引号是0xbb，getstatic的索引号是0xb2
