...
> 在不断学习的过程中，我接触到了“JAVA反射是否很慢”这样的实战，于是乎我就联想到是否可以多维度的模拟性能调优案例，比如搭建一套Spring Web应用，不断模拟用户端请求，创建大量的Bean，通过VisualVM或者Arthas工具查看内存调用情况? 我的思路就展开了，希望能收集到相关调优案例的文章总结，并自己再次模拟
