...
- 前端编译器：责将.java文件编译成.class文件，比如Sun的Javac编译器、Ecipse JDT中的增量式编译器ECJ，目前依然在广泛使用中
- 字节码解释器：负责逐行翻译、解释、执行代码，从字节码 => 汇编码 => 机器码，不改变代码顺序，不做特殊优化，属于早期的解释器，目前已不使用
- 静态提前编译器（AOT编译器）：负责直接将Java代码编译成机器码，在程序执行前进行编译
- 即时编译器（JIT编译器）
